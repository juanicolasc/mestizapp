class ProductsController < ApplicationController
  ...
  def destroy
    if @product.update(:active => false)
        format.html { redirect_to products_url, status: :see_other, notice: "El producto fue anulado exitosamente, ya no se podrá usar para futuros pedidos." }
        format.json { render :show, status: :created, location: @product }
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @product.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def product_params
      params.require(:product).permit(:name, :description, :stock, :active, :price, :kitchen_id, :type_id)
    end
end
